...
export function buildCompanion(logoTexture: THREE.Texture, particleCount = 1400): Companion {
  ...
  // A genuine 3D globe of small triangles enclosing the logo — points
  // distributed over a full sphere shell (Fibonacci sphere, so the coverage
  // is even from every angle) rather than a flat ring, so as it rotates it
  // reads as a real globe with the badge sitting inside it.
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    // Slight random jitter per point so the shell reads as a loose cloud of
    // triangles rather than a perfectly even, artificial-looking lattice.
    const r = globeRadius * (0.94 + Math.random() * 0.14);
